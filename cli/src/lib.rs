...
macro_rules! configure_clapapp {
    ...
    $freshapp.version("1.0.0")
            ...
            .arg(Arg::with_name("nosync")
                .help("By default, safewallet-cli will sync the wallet at startup. Pass --nosync to prevent the automatic sync at startup.")
                .long("nosync")
                .short("n")
                .takes_value(false))
            ...
            .arg(Arg::with_name("COMMAND")
                .help("Command to execute. If a command is not specified, safewallet-cli will start in interactive mode.")
                .required(false)
                .index(1))
...
pub fn report_permission_error() {
    ...
    if home == "/" {
        eprintln!("User {} must have permission to write to '{}.safecoin/' .",
                  user,
                  home);
    } else {
        eprintln!("User {} must have permission to write to '{}/.safecoin/' .",
